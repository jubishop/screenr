...
class KnowledgeTests(unittest.TestCase):
    def setUp(self):
        ...
        # Retain project link targets (CI, architecture, and source references)
        # without copying ignored caches, production data, or Git metadata.
        listed = subprocess.check_output(
            ["git", "ls-files", "--cached", "--others", "--exclude-standard", "-z"], cwd=SOURCE)
        for raw in listed.split(b"\0"):
            if not raw:
                continue
            source = SOURCE / os.fsdecode(raw)
            target = self.repo / os.fsdecode(raw)
